...
It's entertainment + light decision support — not therapy, not diagnosis, and it knows the difference (see [Responsible design](#responsible-design) below).
...
1. Create a new repo (or use an existing one) and add this file as `index.html` at the root.
2. Push to GitHub.
3. Go to **Settings → Pages**.
4. Under **Build and deployment**, set **Source** to `Deploy from a branch`.
5. Set **Branch** to `main` (or `master`) and folder to `/ (root)`.
6. Save. Your site will be live at `https://<your-username>.github.io/<your-repo>/` within a minute or two.

### Option B — Serve from a `/docs` folder
1. Add this file as `docs/index.html`.
2. In **Settings → Pages**, set the branch folder to `/docs` instead of root.
3. Save and wait for the deploy.
